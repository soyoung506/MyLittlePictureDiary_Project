					...
					<c:if test="${diaryTotal !=0}">
						...
						<c:choose>
							<c:when test="${section > 1}">
								<li><a href="${contextPath}/diary/myDiary.do?section=${section-1}&pageNum=5">
						...
						<c:forEach var="page" begin="1" end="${endVal}" step="1">
							<c:choose>
								<c:when test="${page==pageNum}">
									<li><a class="nowPage" href="${contextPath}/diary/myDiary.do?section=${section}&pageNum=${page}">${(section-1)*5+page}</a></li>
								</c:when>
								<c:otherwise>
									<li><a href="${contextPath}/diary/myDiary.do?section=${section}&pageNum=${page}">${(section-1)*5+page}</a></li>
								</c:otherwise>
							</c:choose>									
						</c:forEach>
						...
						<c:choose>
							<c:when test="${section!=total}">
								<li><a href="${contextPath}/diary/myDiary.do?section=${section+1}&pageNum=1">
